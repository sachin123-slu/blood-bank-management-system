 ...
 * @route POST /api/donor/schedule-donation
 * @access Private (Donor)
 */
export const scheduleDonation = async (req, res) => {
  try {
    const donorId = req.donor?.id || req.donor?._id;
    const { preferredDate, preferredTime, facilityId, reason } = req.body;
    ...
    if (selectedDateTime < new Date()) {
      return res.status(400).json({
        success: false,
        message: "Cannot schedule donation in the past"
      });
    }
    ...
    // Add the scheduled donation
    if (!donor.scheduledDonations) {
      donor.scheduledDonations = [];
    }

    donor.scheduledDonations.push({
    ...
    res.status(200).json({
      ...
      donation: {
        date: preferredDate,
        time: preferredTime,
        reason: reason || "Regular donation",
        status: "scheduled"
      }
    });
  } catch (error) {
...
/**
 * @desc Invite a friend to become a donor
 * @route POST /api/donor/invite-friend
 * @access Private (Donor)
 */
export const inviteFriend = async (req, res) => {
  try {
    const donorId = req.donor?.id || req.donor?._id;
    const { friendEmail, friendMessage, referralCode } = req.body;
    ...
    const alreadyInvited = donor.referrals.some(
      ref => ref.email === friendEmail
    );

    if (alreadyInvited) {
      return res.status(400).json({
        success: false,
        message: "You have already invited this email address"
      });
    }

    // Add to referrals
    donor.referrals.push({
      email: friendEmail,
    ...
    console.log(`✉️ Email invitation sent to ${friendEmail}`);
    console.log(`📧 Message: ${friendMessage}`);
